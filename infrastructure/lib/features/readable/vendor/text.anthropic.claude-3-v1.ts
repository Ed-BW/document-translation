	...
	constructor(scope: Construct, id: string, props: props) {
		...
		const invokeBedrock = new tasks.LambdaInvoke(this, "invokeBedrock", {
			...
			payload: sfn.TaskInput.fromObject({
				...
				// Pass correlation IDs to Lambda for structured logging
				jobId: sfn.JsonPath.stringAt("$.jobDetails.id"),
				itemId: sfn.JsonPath.stringAt("$.jobDetails.itemId"),
				executionArn: sfn.JsonPath.stringAt("$$.Execution.Id"),
			}),
			// Add retry configuration - let Lambda handle retries internally
			// Step Functions will only retry on Lambda service errors
